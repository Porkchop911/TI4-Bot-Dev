...
pub fn baseline_bounds() -> BTreeMap<String, (f64, f64)> {
    ...
    // `share_SPACE_COMBAT_RESOLVED` falls with it. Fewer ships get built, so there is less to fight
    // with and objectives arrive later.
    //
    // Two other fixes in this batch move nothing on their own -- Non-Euclidean Shielding needs the
    // technology, and the Harrow gate needs L1Z1X facing a shield.
    //
    ...
    //
    // Dilution again, and small: every value already sat inside v20 and the intervals move by
    // thousandths in both directions. Three of the four open a new question -- the Crown after
    // every tactical action, the Orb after every activation, the Neuraloop on every objective
    // reveal -- so the decider stream lengthens wherever the relic is held, and the seeds shift
    // under it. `completion` is untouched and `vp_pace` moves by 0.0006.
    //
    ...
    //
    // This one is a change in *play*, not dilution, and the signature says so. The six action-mix
    // shares move by thousandths in both directions -- no new event type, so nothing lengthens the
    // stream -- while `vp_pace` rises [0.386, 0.441] -> [0.411, 0.457], `score_spread` rises
    // [1.623, 2.089] -> [1.743, 2.109] and `faction_differentiation` rises [0.457, 0.980] ->
    // [0.528, 1.043]. Research that used to be unaffordable now happens, so seats reach their
    // objectives sooner and the seats holding these cards pull away from those that do not.
    //
    // Every current value still sat inside the v17 intervals; the intervals themselves moved.
    //
    // v17 — 2026-08-31. Three relic ACTIONs gained a decider: Enigmatic Device, Maw of Worlds and
    // The Codex. Enigmatic Device also stopped being offered when its six resources cannot be paid
    // (22.3), which changes what bots are asked and therefore the stream.
    //
    // Every current value already sat inside the v16 intervals; what forced this is the
    // protocol-integrity check, which requires the recorded bounds to be exactly what the tree
    // recomputes and is deliberately stricter than the value gate.
    //
    // v16 — 2026-08-31. Lie in Wait, the last printed action card, needs a TRANSACTION_RESOLVED
    // event so its window can fire. A new event type lengthens the stream, so every action-mix
    // share dilutes; two fell below their floors, one of them by 0.00004.
    //
    // The card itself is played by nobody in this suite -- there is no bot scoring for it -- so the
    // movement in vp_pace and score_spread is the ordinary drift of a shifted decider stream rather
    // than the card doing anything.
    //
    ...
    // the reaction tables can bind to (Waylay).
    //
    // - Direct Hit: after another player's ship uses SUSTAIN DAMAGE on a hit the holder
    //   produced, the holder destroys that ship (the removal is announced as a first-class
    //   `SHIP_DESTROYED` event).
    // - Rout: the opponent must announce a retreat at the start of the round's retreat
    //   step, if able — "stay" leaves the table for them.
    // - Waylay: the holder's own barrage hits land on all ships, not just fighters, so a
    //   casualty the silent AFB path would have absorbed in a fighter is now *chosen*.
    //
    // Every point estimate stays inside its v11 interval, but the bootstrap intervals
    // themselves moved: the bots now hold and play these cards, so fights that used to
    // evaporate on a fighter trade can now split across ship types, and the new event
    // dilutes the stream denominators a little further. `faction_differentiation`'s lower
    // bound rises (0.431 -> 0.541) and `score_spread`'s upper bound rises (2.091 -> 2.256) —
    // the new cards give combats more ways to end differently; the action-label intervals
    // drift by a few thousandths either way, and `vp_pace`'s lower bound rises (0.404 ->
    // 0.419). `completion` stays the strict 1.0 invariant. The protocol-integrity check is
    // what forced the move, as in v8 -> v9.
    //
    // Approved by the project owner (the engine-completion handoff's standing instruction to
    // re-baseline when this card group lands); v11 values preserved side by side in
    // plans/evidence/M08-021.md.
    //
    ...
    // cancelled in its WHEN window, the card is still spent, and its effect never runs.
    // The reaction slot that owns the triggering event performs the cancellation, and the
    // "other than 'Sabotage'" guard keeps the copies from cancelling each other.
    //
    // The bots now hold and play Sabotage, so a card play the v8 bots let stand can now be
    // interrupted. The shifts are modest: `faction_differentiation` and `score_spread`
    // widen further (a play can now be undone, another way for endings to split), the
    // action-label shares drift by a few thousandths in both directions, and `completion`
    // stays the strict 1.0 invariant — a cancelled play spends its card and the game still
    // ends cleanly. The protocol-integrity check is what forced the move: the recomputed
    // bounds no longer match the v8 transcription, even though every v9 value sits inside
    // its v8 interval.
    //
    // Approved by the project owner (the engine-completion handoff's standing instruction to
    // re-baseline when this card group lands); v8 values preserved side by side in
    // plans/evidence/M08-021.md.
    //
    ...
    // - Sol's Versatile grants its extra status token, which had been computed and never applied.
    //
    // The movement is modest. `vp_pace` falls [0.409877, 0.459259] -> [0.388426, 0.443210] and
    // `score_spread` rises [1.788310, 2.162748] -> [1.916356, 2.308221]: enforcing capacity at the
    // combat takes ground forces off planets that used to be captured, which costs points and
    // widens the gap between seats that lost a carrier and seats that did not. `share_SHIP_MOVED`
    // rises [0.045356, 0.049027] -> [0.047297, 0.051156]. Every other share moves by less than its
    // own interval width. All thirty games still finish cleanly.
    //
    // Recorded against the committed tree, not the working tree. A first attempt derived these in a
    // checkout carrying ~50 files of other sessions' uncommitted work and produced a much larger
    // apparent shift -- activity up across three shares -- which belonged to that work and not to
    // the fixes above. Bounds recomputed in a clean worktree at the commit they ship with; anyone
    // repeating this must do the same or they will record somebody else's tree.
    //
    ...
    bounds.insert(
        "vp_pace".to_owned(),
        (0.382_098_765_432_098_86, 0.440_740_740_740_740_9),
    );
    ...
    bounds.insert(
        "score_spread".to_owned(),
        (1.732_608_289_075_065_6, 2.095_391_879_306_217),
    );
    // V3: the spec's across-faction quantity — re-deriven with the same baseline run.
    bounds.insert(
        "faction_differentiation".to_owned(),
        (0.792_947_307_264_969_1, 1.296_493_371_262_805_5),
    );
    bounds.insert(
        "share_INVASION_RESOLVED".to_owned(),
        (0.018_526_455_885_292_34, 0.019_718_397_516_748_88),
    );
    bounds.insert(
        "share_PRODUCTION_RESOLVED".to_owned(),
        (0.034_562_557_145_463_996, 0.035_824_061_564_130_79),
    );
    bounds.insert(
        "share_SHIP_MOVED".to_owned(),
        (0.045_455_856_853_414_81, 0.048_089_926_658_027_56),
    );
    bounds.insert(
        "share_SPACE_COMBAT_RESOLVED".to_owned(),
        (0.004_361_380_643_524_044, 0.005_064_738_901_398_582_6),
    );
    bounds.insert(
        "share_SYSTEM_ACTIVATED".to_owned(),
        (0.068_426_884_612_037_43, 0.070_861_630_789_963_87),
    );
    bounds.insert(
        "share_TACTICAL_ACTION_BEGAN".to_owned(),
        (0.033_862_803_224_706_355, 0.035_053_072_348_579_87),
    );
    bounds
}
