...
/// The version-5 registry: version 4 with the actor-inventory family appended.
///
/// OBS-004a. Appended rather than inserted, for the reason every prior version gives: every column
/// a v4 vocabulary allocated keeps its index, so the change is additive in the one way that matters.
const OOV_FAMILIES_V5: [&str; 42] = {
    let mut families = [""; 42];
    let mut index = 0;
    while index < OOV_FAMILIES_V4.len() {
        families[index] = OOV_FAMILIES_V4[index];
        index += 1;
    }
    families[41] = "actor-inventory";
    families
};

/// The version-6 registry: version 5 with the opponent-slot family appended.
///
/// OBS-005. Appended rather than inserted, for the reason every prior version gives: every column
/// a v5 vocabulary allocated keeps its index, so the change is additive in the one way that matters.
const OOV_FAMILIES_V6: [&str; 43] = {
    let mut families = [""; 43];
    let mut index = 0;
    while index < OOV_FAMILIES_V5.len() {
        families[index] = OOV_FAMILIES_V5[index];
        index += 1;
    }
    families[42] = "opponent-slot";
    families
};
...
/// The version-8 registry: version 7 with the combat decision-surface family appended.
///
/// OBS-008b2. Appended rather than inserted, for the reason every prior version gives: every column
/// a v7 vocabulary allocated keeps its index, so the change is additive in the one way that matters.
const OOV_FAMILIES_V8: [&str; 45] = {
    let mut families = [""; 45];
    let mut index = 0;
    while index < OOV_FAMILIES_V7.len() {
        families[index] = OOV_FAMILIES_V7[index];
        index += 1;
    }
    families[44] = "combat";
    families
...
/// The version-9 registry: version 8 with the strategy decision-surface family appended.
///
/// OBS-008d1. Appended rather than inserted, for the reason every prior version gives: every column
/// a v8 vocabulary allocated keeps its index, so the change is additive in the one way that matters.
const OOV_FAMILIES_V9: [&str; 46] = {
    let mut families = [""; 46];
    let mut index = 0;
    while index < OOV_FAMILIES_V8.len() {
        families[index] = OOV_FAMILIES_V8[index];
        index += 1;
    }
    families[45] = "strategy";
    families
};

/// The version-10 registry: version 9 with the content decision-surface family appended.
///
/// OBS-008e/f/g/h/i. Appended rather than inserted, for the reason every prior version gives: every
/// column a v9 vocabulary allocated keeps its index, so the change is additive in the one way that
/// matters.
const OOV_FAMILIES_V10: [&str; 47] = {
    let mut families = [""; 47];
    let mut index = 0;
    while index < OOV_FAMILIES_V9.len() {
        families[index] = OOV_FAMILIES_V9[index];
        index += 1;
    }
    families[46] = "content";
    families
...
/// The pinned fingerprint of the ordered version-5 registry.
pub const OOV_FAMILIES_V5_FINGERPRINT: &str =
    "cf95e81daf11c00c272f25d0d0e57a506aaef731d812526df2dcec75ad7f0b31";

/// The pinned fingerprint of the ordered version-6 registry.
...
/// The pinned fingerprint of the ordered version-8 registry.
pub const OOV_FAMILIES_V8_FINGERPRINT: &str =
    "58abcdcf1c0e2fa21f6f8b97b6c9d233e489f92d7db57009e2fb11d8272bff59";

/// The pinned fingerprint of the ordered version-9 registry.
pub const OOV_FAMILIES_V9_FINGERPRINT: &str =
    "446509f90c8d17270f081b4cd86d79364ff30e40b51da6a49b2b513b636731a5";
...
impl Vocabulary {
    ...
    ///
    /// In addition to the current registry, this accepts version 5 with its exact frozen reserved
    /// prefix. Version 6 only appended the `opponent-slot` OOV family; a v5 actor therefore keeps
    /// every stored column at its original address, while new opponent-slot facts safely fall
    /// back to the global OOV row. This does not relabel, rewrite, append to, or train the
    /// vocabulary.
    ///
    /// # Errors
    /// [`LoadError::Json`] for malformed input, or [`LoadError::Invalid`] if neither the exact v5
    ...
    fn validate_versioned(
        &self,
        allow_prior_version_inference: bool,
    ) -> Result<(), VocabularyError> {
...
mod tests {
    ...
    fn the_reserved_order_is_pinned_and_each_version_preserves_every_earlier_index() {
        ...
        assert_eq!(
            registry_fingerprint(&OOV_FAMILIES_V5),
            OOV_FAMILIES_V5_FINGERPRINT,
            "the ordered v5 registry changed"
        );
        assert_eq!(
            registry_fingerprint(&OOV_FAMILIES_V6),
        ...
        assert_eq!(
            registry_fingerprint(&OOV_FAMILIES_V8),
            OOV_FAMILIES_V8_FINGERPRINT,
            "the ordered v8 registry changed"
        );
        assert_eq!(
            registry_fingerprint(&OOV_FAMILIES_V9),
            OOV_FAMILIES_V9_FINGERPRINT,
            "the ordered v9 registry changed"
        );
        assert_eq!(
            registry_fingerprint(&OOV_FAMILIES_V10),
        ...
        assert_eq!(OOV_FAMILIES_V4.len(), OOV_FAMILIES_V3.len() + 1);
        for (index, family) in OOV_FAMILIES_V3.iter().enumerate() {
            assert_eq!(
                OOV_FAMILIES_V4[index], *family,
                "v4 moved the v3 reserved column at index {index}"
            );
        }
        assert_eq!(
            OOV_FAMILIES_V4[OOV_FAMILIES_V3.len()],
            crate::projection::ACTION_FAMILY,
            "the appended family is not the action-feasibility namespace"
        );

        assert_eq!(OOV_FAMILIES_V5.len(), OOV_FAMILIES_V4.len() + 1);
        for (index, family) in OOV_FAMILIES_V4.iter().enumerate() {
            assert_eq!(
                OOV_FAMILIES_V5[index], *family,
                "v5 moved the v4 reserved column at index {index}"
            );
        }
        assert_eq!(
            OOV_FAMILIES_V5[OOV_FAMILIES_V4.len()],
            "actor-inventory",
            "the appended family is not the actor-inventory namespace"
        );

        assert_eq!(OOV_FAMILIES_V6.len(), OOV_FAMILIES_V5.len() + 1);
        for (index, family) in OOV_FAMILIES_V5.iter().enumerate() {
            assert_eq!(
                OOV_FAMILIES_V6[index], *family,
                "v6 moved the v5 reserved column at index {index}"
            );
        }
        assert_eq!(
            OOV_FAMILIES_V6[OOV_FAMILIES_V5.len()],
            "opponent-slot",
        ...
        assert_eq!(OOV_FAMILIES_V8.len(), OOV_FAMILIES_V7.len() + 1);
        for (index, family) in OOV_FAMILIES_V7.iter().enumerate() {
            assert_eq!(
                OOV_FAMILIES_V8[index], *family,
                "v8 moved the v7 reserved column at index {index}"
            );
        }
        assert_eq!(
            OOV_FAMILIES_V8[OOV_FAMILIES_V7.len()],
            "combat",
            "the appended family is not the combat decision-surface namespace"
        );

        assert_eq!(OOV_FAMILIES_V9.len(), OOV_FAMILIES_V8.len() + 1);
        for (index, family) in OOV_FAMILIES_V8.iter().enumerate() {
            assert_eq!(
                OOV_FAMILIES_V9[index], *family,
                "v9 moved the v8 reserved column at index {index}"
            );
        }
        assert_eq!(
            OOV_FAMILIES_V9[OOV_FAMILIES_V8.len()],
            "strategy",
            "the appended family is not the strategy decision-surface namespace"
        );

        assert_eq!(OOV_FAMILIES_V10.len(), OOV_FAMILIES_V9.len() + 1);
        for (index, family) in OOV_FAMILIES_V9.iter().enumerate() {
            assert_eq!(
                OOV_FAMILIES_V10[index], *family,
                "v10 moved the v9 reserved column at index {index}"
            );
        }
        assert_eq!(
            OOV_FAMILIES_V10[OOV_FAMILIES_V9.len()],
            "content",
    ...
    #[test]
    fn version_nine_loads_for_inference_without_renumbering_its_columns() {
        let mut vocabulary = Vocabulary::build(sample()).expect("builds");
        ...
        let loaded = Vocabulary::from_json_for_inference(&text).expect("v9 inference loads");
        assert_eq!(loaded.oov_registry_version(), 9);
        assert_eq!(loaded.column_of(&assigned_name), assigned_column);
        assert_eq!(
            loaded.column_of("content:new-current-only-fact"),
            GLOBAL_OOV_COLUMN,
    ...
    #[test]
    fn version_eight_remains_refused_for_inference() {
        // v10 only extends the one-version-back window to v9 (OBS-008e/f/g/h/i); v8 is now two
        // versions back and must be refused exactly as v2-v7 already are.
        let mut vocabulary = Vocabulary::build(sample()).expect("builds");
        vocabulary.oov_registry_version = 8;
        let error = Vocabulary::from_json_for_inference(&vocabulary.to_json().expect("json"))
            .expect_err("v8 is not an inference-compatible layout under the v10 registry");
        assert!(matches!(
            error,
            LoadError::Invalid(VocabularyError::UnsupportedRegistry { .. })
        ));
    ...
    #[test]
    fn version_seven_remains_refused_for_inference() {
        let mut vocabulary = Vocabulary::build(sample()).expect("builds");
        vocabulary.oov_registry_version = 7;
        let error = Vocabulary::from_json_for_inference(&vocabulary.to_json().expect("json"))
            .expect_err("v7 is not an inference-compatible layout");
        assert!(matches!(
            error,
            LoadError::Invalid(VocabularyError::UnsupportedRegistry { .. })
        ));
    ...
    #[test]
    fn version_six_remains_refused_for_inference() {
        let mut vocabulary = Vocabulary::build(sample()).expect("builds");
        vocabulary.oov_registry_version = 6;
        let error = Vocabulary::from_json_for_inference(&vocabulary.to_json().expect("json"))
            .expect_err("v6 is not an inference-compatible layout");
        assert!(matches!(
            error,
            LoadError::Invalid(VocabularyError::UnsupportedRegistry { .. })
        ));
    ...
    fn version_five_remains_refused_for_inference() {
        ...
        let error = Vocabulary::from_json_for_inference(&vocabulary.to_json().expect("json"))
            .expect_err("v5 is not an inference-compatible layout");
        assert!(matches!(
            error,
            LoadError::Invalid(VocabularyError::UnsupportedRegistry { .. })
        ));
    ...
    #[test]
    fn version_three_remains_refused_for_inference() {
        let mut vocabulary = Vocabulary::build(sample()).expect("builds");
        vocabulary.oov_registry_version = 3;
        let error = Vocabulary::from_json_for_inference(&vocabulary.to_json().expect("json"))
            .expect_err("v3 is not an inference-compatible layout");
        assert!(matches!(
            error,
            LoadError::Invalid(VocabularyError::UnsupportedRegistry { .. })
        ));
