...
fn mechanics() -> Vec<(&'static str, &'static str, Evidence)> {
    vec![
        // -- round structure ------------------------------------------------------------------
        ("structure", "round begins", Evidence::Event(&["ROUND_BEGAN"])),
        ("structure", "strategy phase: cards chosen", Evidence::Event(&["STRATEGY_CARD_CHOSEN"])),
        ("structure", "action phase", Evidence::Event(&["ACTION_PHASE_BEGAN"])),
        ("structure", "status phase", Evidence::Event(&["STATUS_PHASE_BEGAN"])),
        ("structure", "status: scoring window", Evidence::Event(&["STATUS_SCORING_BEGAN"])),
        ("structure", "status: bookkeeping", Evidence::Event(&["STATUS_BOOKKEEPING_RESOLVED"])),
        ("structure", "agenda phase", Evidence::Event(&["AGENDA_PHASE_BEGAN"])),
        ("structure", "passing", Evidence::Event(&["PLAYER_PASSED", "TURN_PASSED"])),
        ("structure", "game end", Evidence::Event(&["GAME_FINISHED"])),
        // -- the tactical action --------------------------------------------------------------
        ("tactical", "tactical action", Evidence::Event(&["TACTICAL_ACTION_BEGAN"])),
        ("tactical", "system activation", Evidence::Event(&["SYSTEM_ACTIVATED"])),
        ("tactical", "movement", Evidence::Event(&["SHIP_MOVED"])),
        ("tactical", "space combat", Evidence::Event(&["SPACE_COMBAT_STARTED"])),
        ("tactical", "combat rounds fought", Evidence::Event(&["COMBAT_ROUND_STARTED"])),
        ("tactical", "space combat resolved", Evidence::Event(&["SPACE_COMBAT_RESOLVED"])),
        ("tactical", "invasion", Evidence::Event(&["INVASION_BEGAN"])),
        ("tactical", "planet control taken", Evidence::Event(&["PLANET_CONTROL_GAINED"])),
        ("tactical", "production", Evidence::Event(&["PRODUCTION_RESOLVED", "PRODUCTION_USED"])),
        ("tactical", "gravity rift losses", Evidence::Event(&["SHIP_LOST_TO_GRAVITY_RIFT"])),
        // -- strategy cards -------------------------------------------------------------------
        ("strategy", "strategic action", Evidence::Event(&["STRATEGIC_ACTION_BEGAN"])),
        ("strategy", "secondary followed", Evidence::Event(&["STRATEGY_SECONDARY_FOLLOWED"])),
        ("strategy", "secondary declined", Evidence::Event(&["STRATEGY_SECONDARY_DECLINED"])),
        ("strategy", "command tokens gained", Evidence::Event(&["COMMAND_TOKENS_GAINED", "COMMAND_TOKEN_GAINED"])),
        // -- economy --------------------------------------------------------------------------
        ("economy", "transaction opened", Evidence::Event(&["TRANSACTION_OPENED", "TRANSACTION_OFFERED"])),
        ("economy", "transaction agreed", Evidence::Event(&["TRANSACTION"])),
        ("economy", "trade goods held", Evidence::State(|s| s.players.iter().any(|p| p.trade_goods > 0))),
        ("economy", "commodities held", Evidence::State(|s| s.players.iter().any(|p| p.commodities > 0))),
        // -- cards ----------------------------------------------------------------------------
        ("cards", "action card played", Evidence::Event(&["ACTION_CARD_PLAYED", "CARD_PLAYED"])),
        ("cards", "component action", Evidence::Event(&["COMPONENT_ACTION_RESOLVED"])),
        ("cards", "promissory: ceasefire used", Evidence::Event(&["CEASEFIRE_USED"])),
        ("cards", "promissory: support lent", Evidence::State(|s| !s.support_holders.is_empty())),
        // -- agenda ---------------------------------------------------------------------------
        ("agenda", "agenda revealed", Evidence::Event(&["AGENDA_REVEALED"])),
        ("agenda", "votes cast", Evidence::Event(&["VOTES_CAST"])),
        ("agenda", "agenda resolved", Evidence::Event(&["AGENDA_RESOLVED"])),
        ("agenda", "law in play", Evidence::State(|s| !s.laws.is_empty())),
        // -- progression ----------------------------------------------------------------------
        ("progression", "custodians lifted", Evidence::State(|s| s.custodians_removed)),
        ("progression", "technology researched", Evidence::State(|s| s.players.iter().any(|p| p.technologies.len() > 3))),
        ("progression", "unit upgrade researched", Evidence::State(|s| {
            s.players.iter().any(|p| p.technologies.iter().any(|t| {
                let id = t.to_string();
                id.contains('2') || id.ends_with("ii")
            }))
        })),
        ("progression", "objective scored", Evidence::Event(&["OBJECTIVE_SCORED"])),
        ("progression", "relic acquired", Evidence::State(|s| s.players.iter().any(|p| !p.relics.is_empty()))),
        ("progression", "relic fragments found", Evidence::State(|s| s.players.iter().any(|p| p.relic_fragments.values().any(|n| *n > 0)))),
        ("progression", "leaders unlocked", Evidence::State(|s| s.players.iter().any(|p| !p.leaders.is_empty()))),
        ("progression", "exploration attachments", Evidence::State(|s| !s.planet_attachments.is_empty())),
        ("progression", "secret objectives scored", Evidence::State(|s| s.scored_objectives.values().any(|set| !set.is_empty()))),
        ("progression", "victory points scored", Evidence::State(|s| s.players.iter().any(|p| p.victory_points > 0))),
    ]
}
