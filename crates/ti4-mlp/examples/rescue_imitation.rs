...
fn play(
    ...
    let handles: std::rc::Rc<
        std::cell::RefCell<BTreeMap<PlayerId, std::rc::Rc<std::cell::RefCell<Vec<ti4_mlp::bot::PpoRecord>>>>>,
...
fn main() {
        ...
        for rotation in 0..FACTIONS.len() {
            ...
            for player in failed {
                ...
                for attempt in 0..attempts {
                    ...
                    let outcome = play(&setup, seed, rotation, Some((&player, temperature, stream)));
    ...
    let outcome = ti4_mlp::distill::train(&mut trained, train_samples, validation, settings, |epoch| {
        println!(
    ...
    let bundle = ti4_mlp::bundle::write(
        ...
        &ti4_mlp::bundle::Provenance {
            source: format!("rescue imitation over {} first-divergence decisions from {bundle_path}", train_samples.len()),
