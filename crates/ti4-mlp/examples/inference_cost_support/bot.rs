...
pub struct MlpBot {
    pub profile: std::rc::Rc<std::cell::RefCell<super::Profile>>,
    pub verify: bool,
    pub cache_columns: bool,
    column_cache: std::collections::HashMap<ti4_policy::intern::FeatureKey, (usize, bool)>,
    ...
    ppo_mode: Option<ti4_mlp::bundle::CriticMode>,
...
impl MlpBot {
    ...
    fn sparse_from(
        ...
        for (key, value) in vector {
            ...
            let (column, assigned) = if self.cache_columns {
                *self.column_cache.entry(*key).or_insert_with(|| {
                    (
                        self.vocabulary.column_of_key(*key),
                        self.vocabulary.is_assigned_key(*key),
                    )
                })
            } else {
                (
            ...
            if self.verify {
                assert_eq!(column, self.vocabulary.column_of_key(*key));
                assert_eq!(assigned, self.vocabulary.is_assigned_key(*key));
            }
            if assigned {
...
impl MlpBot {
    /// Record every decision as a PPO [`ti4_mlp::ppo::Step`], for an update built from self-play.
    ///
    ...
    #[must_use]
    pub const fn recording_ppo(mut self, mode: ti4_mlp::bundle::CriticMode) -> Self {
    ...
    /// Record one decision as a PPO [`ti4_mlp::ppo::Step`], at the only moment its behaviour
    /// quantities exist.
    ///
    ...
    fn record(
        ...
        choice: &Choice,
        seen: &SeatObservation<'_>,
        mode: ti4_mlp::bundle::CriticMode,
        probabilities: &[f64],
        ...
        let (critic, behaviour_value) = if matches!(mode, ti4_mlp::bundle::CriticMode::BatchMean) {
            (None, None)
        } else {
            let vector =
                ti4_policy::critic::critic_vector(seen, ti4_policy::critic::CriticFeatures::full());
            let critic = ti4_mlp::CriticInput::new(&vector, &self.vocabulary);
            self.profile
                .borrow_mut()
                .add("critic_features", phase_started.elapsed());
            let phase_started = std::time::Instant::now();
            let value = self
                .actor
            ...
            self.profile
                .borrow_mut()
                .add("critic_forward", phase_started.elapsed());
            if !value.is_finite() {
        ...
        self.profile
            .borrow_mut()
            .add("record_progress", phase_started.elapsed());
        Ok(())
    }
    ...
    fn decide(
        ...
        let vectors = super::projection::mlp_choice_features(
            &self.profile,
            seen.observed(),
            choice,
            &choice.player,
        ...
        self.profile
            .borrow_mut()
            .add("features", phase_started.elapsed());
        if self.verify {
            let expected = ti4_policy::projection::mlp_choice_features(
                seen.observed(),
                choice,
                &choice.player,
                &held,
        ...
        self.profile
            .borrow_mut()
            .add("vocabulary", phase_started.elapsed());
        self.profile.borrow_mut().features += vectors.iter().map(|v| v.len()).sum::<usize>();
        self.profile.borrow_mut().options += options.len();
        ...
        let probabilities = match super::profile_probabilities(
            &self.actor,
            &options,
            head,
            self.row,
            ...
            Err(error) => {
                ...
                self.counters.fallbacks.fetch_add(1, Ordering::Relaxed);
                eprintln!("MLP inference failed on head {head} ({error}); refusing the decision");
        ...
        self.profile
            .borrow_mut()
            .add("sampling", phase_started.elapsed());
