...
/// How the five opponents were sampled when a line was recorded.
///
/// The two corpora differ and neither records it. `build_positive_corpus` puts every seat at the
/// corpus temperature; `rescue_search` holds the opponents greedy and heats only the searching seat.
/// Replaying with the wrong convention faces different opponents, so the line does not exist and the
/// demonstration is silently lost.
    ...
    let played = ti4_training::rollout::audit_game_with_deciders(
        ...
        |seated, baselines| {
            ...
            for (index, (player, faction)) in seated.iter().enumerate() {
                ...
                let heated = mine || opponents == Opponents::SameTemperature;
                let stream = trajectory
                    .seed
                    .wrapping_mul(1_000_003)
                    .wrapping_add(u64::try_from(index).unwrap_or(0))
                    .wrapping_add(if heated {
                ...
                let bot =
                    ti4_mlp::bot::MlpBot::sharing(actor, table.vocabulary.clone(), row, stream)
                        .at_temperature(if heated { temperature } else { 0.001 })
                        .from_setup(baseline);
                if mine {
...
fn main() {
    ...
    // The rescued corpus is the only data from positions the policy actually loses. The ordinary
    // corpus is the stability distribution; this is the new information.
    let rescued_dir = argument("--rescued");
    // Share of each epoch drawn from the rescued corpus. Separate from the weighting, because how
    // much rescued data to *see* and how much to *weight* it are different questions.
    let rescued_share: f64 = number("--rescued-share", 0.5);
    ...
    // Replay must use the policy the corpus was generated by, or the recorded lines do not exist.
    // Training may start from a *different* policy -- typically a better one produced by an earlier
    // round of this same loop. Without separating them, every round would have to restart from the
    // original champion and discard its own gains.
    let frozen = match argument("--replay-bundle") {
        Some(path) => {
            ti4_mlp::bundle::read(std::path::Path::new(&path))
                .unwrap_or_else(|error| refuse(&format!("reading {path}: {error}")))
                .actor
        }
        None => actor.inference_copy(),
    };
    ...
    // ---- load the corpus ---------------------------------------------------------------------
    let load = |directory: &str| -> BTreeMap<String, Vec<Trajectory>> {
        let mut out: BTreeMap<String, Vec<Trajectory>> = BTreeMap::new();
    ...
    let ordinary = load(&corpus_dir);
    let rescued = rescued_dir.as_ref().map(|d| load(d)).unwrap_or_default();
    let ordinary_total: usize = ordinary.values().map(Vec::len).sum();
    let rescued_total: usize = rescued.values().map(Vec::len).sum();
    if ordinary_total == 0 {
        refuse("the ordinary corpus is empty");
    }
    let share = if rescued_total == 0 {
        0.0
    } else {
        rescued_share
    };
    ...
    println!("  ordinary   {corpus_dir}, {ordinary_total} trajectories");
    if let Some(directory) = &rescued_dir {
        println!(
            "  rescued    {directory}, {rescued_total} trajectories, {:.0}% of each epoch",
            share * 100.0
        );
    }
    println!("  per epoch  {per_epoch}, balanced across factions and sources");
    println!("  step       {learning_rate}");
    if argument("--replay-bundle").is_some() {
        println!(
            "  replay     {} (the corpus generator)",
            argument("--replay-bundle").unwrap_or_default()
        );
    }
    println!();
    ...
    for epoch in 1..=epochs {
        ...
        let mut drawn: Vec<(Trajectory, Opponents)> = Vec::new();
        #[expect(
            clippy::cast_possible_truncation,
            clippy::cast_sign_loss,
            reason = "a share of a per-epoch count"
        )]
        ...
        let mut take = |rows: &Vec<Trajectory>,
            ...
            // A stride rather than a prefix, so an epoch sees the whole seed range, and an offset
            // that moves with the epoch so successive epochs see different lines.
            let stride = (rows.len() / want).max(1);
            let offset = epoch.wrapping_mul(7919) % rows.len();
            for index in 0..want.min(rows.len()) {
                out.push((
                    rows[(offset + index * stride) % rows.len()].clone(),
                    convention,
                ));
            }
        };
        for faction in FACTIONS {
            if let Some(rows) = ordinary.get(faction) {
                take(rows, ordinary_each, Opponents::SameTemperature, &mut drawn);
            }
            if let Some(rows) = rescued.get(faction) {
                take(rows, rescued_each, Opponents::Greedy, &mut drawn);
            }
        }
        ...
        for chunk in drawn.chunks(batch) {
            ...
            let harvest: Vec<(Vec<Demo>, usize)> = chunk
                ...
                .map(|(local, piece)| {
                    ...
                    let mut bad = 0usize;
                    for (trajectory, convention) in piece {
                        match demonstrate(&table, &local, &trajectory, convention) {
