...
**Accepted.** F-M09-024a-4 is resolved by structural provenance validation before arithmetic and a
total, overflow-safe integer capacity calculation. Independent review of the complete correction
frontier found no further actionable issues; F-M09-024a-1 through F-M09-024a-4 are closed.

Independent gates: focused vocabulary **24/0**; complete workspace **1,403/0**; scoped Clippy has
no package-owned warning and only the documented pre-existing engine warning at `game.rs:1260`;
scoped rustfmt and `git diff --check` clean. M09-024a is accepted and M09-024b is dependency-ready.

## Independent Tier-C recheck of `7eb0722` (2026-08-25)

**Changes required.** F-M09-024a-3 is resolved: the persisted allocation-time count correctly
decouples fixed physical capacity from post-append logical size, and the new threshold/full-capacity
round trips preserve the layout. The recheck found one new HIGH blocker, F-M09-024a-4:
`allocated_for` is untrusted JSON but reaches `capacity_for` before its structural bound check.
`usize::MAX` consequently panics on the granularity multiplication instead of producing
`LoadError::Invalid`.

An independent temporary regression reproduced the panic at `vocabulary.rs:682` (`attempt to
multiply with overflow`, 0/1); it was then removed and the submitted source restored. Independent
green gates on the submitted tree: focused vocabulary **21/0**, full policy library **157/0**;
scoped Clippy only reports the documented pre-existing engine warning at `game.rs:1260`.

M09-024a remains open and M09-024b blocked pending structural provenance validation before
arithmetic, overflow-safe `capacity_for`, and a retained malformed-JSON no-unwind regression.
