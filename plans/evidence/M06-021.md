...
| **Depends** | M06-020 (reopened extension) |
| **Review tier** | **C** — scoring legality and hidden information |
| **Reviewer** | Codex frontier review, 2026-08-21 — independent of the implementer |
| **Status** | implemented and merged; **review complete with critical finding; not accepted** |
| **Branch** | `codex/mlp-policy` |
| **Commit** | `0d751a8` (evidence `334ef83`, revised `HEAD`) |
| **Normative source** | FFG *Living Rules Reference 2.0*, rule 61.7, plus printed objective timing; Python parity is explicitly out of scope |
...
3. **The normative rules source was not recorded.** Python is no longer a behavioral
   oracle for this project. The independent review instead checked the official
   rules and found the timing defect below.
4. **No independent review before merge.** A tier C review has now been performed,
   but its critical finding blocks acceptance and M06-022 until M06-021a resolves it.
...
## Claimed invariants in the merged implementation
...
- A feat satisfies only the turn it was recorded in, so a combat won in round 1
  cannot pay out in round 4. This is insufficient event scoping, not the accepted
  rule boundary; see the review finding.
...
## Independent tier C review — 2026-08-21

Normative source: official [FFG *Living Rules Reference 2.0*](https://images-cdn.fantasyflightgames.com/filer_public/51/55/51552c7f-c05c-445b-84bf-4b073456d008/ti10_pok_living_rules_reference_20_web.pdf),
rule 61.7. Python behavior was inspected during diagnosis but, by explicit project
decision, is historical context and not an acceptance target.

**Critical finding — open.** `Game::advance_turn` coalesces every action feat into
one late scoring offer. Rule 61.7 permits any number of objectives during an action
turn or agenda phase, limits scoring to one objective during or after each combat,
and explicitly permits an objective in both the space and ground combat of one
tactical action. The implementation can therefore suppress a legal second combat
window and delay non-combat event objectives beyond their printed timing.

**Major finding — open.** Become a Martyr is tested as a persistent later board
position. Its condition is losing control of a home-system planet; the offer must
be tied to that control-loss occurrence so later recapture or an unrelated later
turn cannot manufacture or erase the scoring opportunity.

**Required resolution.** M06-021a introduces occurrence-scoped combat-end,
anti-fighter-barrage, space-cannon, bombardment, planet-control, pass, and agenda
windows; preserves one objective per combat while
allowing all eligible non-combat action/agenda objectives; adds separate
space/ground-combat, multi-agenda, event-attribution and hidden-hand tests; and
receives a fresh tier C review. Existing measurements do not close these findings.
...
- Tier C review is complete, with the critical and major findings above open.
  **M06-021a blocks M06-022.**
- Six of the fourteen scored 0% in the historical 150-game run, but rarity vs
  defect is unclassified. Zero-rate and VP conclusions must be rerun after
  M06-021a because the measured mechanism has known timing defects.

## Caveat

The pool is reproducible from committed code (`examples/generate_pool.rs`,
`--seed 777`). The checkpoint is not reproducible; M09-020 in `docs/MLP_PLAN.md`
must archive it as a bounded durable fixture before distillation.
