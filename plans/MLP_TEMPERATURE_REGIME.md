...
| **1.0** | **Training, and the distillation student.** The distribution PPO's importance ratio is computed against; pinned for distillation. | `ppo_update` (implicit until 2026-09-01), `opening_failures`, `opening_plan` |
...
**1.0 — training.** Two reasons, and the second is not optional:

- Exploration. PPO is on-policy; the batch is whatever the policy sampled. Sharpening the
  distribution narrows what the update can learn from.
- **The importance ratio.** `MlpBot` has exactly one `probabilities()` call, so the acting
  distribution *is* the recorded behaviour distribution. Setting the sampling temperature without
  recording the same value would make every ratio in every batch a comparison between two different
  distributions. `--temperature` (added 2026-09-01) is safe precisely because it feeds that single
  call.
...
model, before any evidence justified it.
...
- **Never** set a sampling temperature on a PPO run without confirming the recorded behaviour
  probabilities use the same value.
