...
- Branch: `wp/m00-009e-event-exporter`, based on M00-009d package commit `6a90f24`.
- Last completed package: M00-009e — executable resolved-event projection
  (`plans/evidence/M00-009e.md`).
- M00-008 fixture-selection and M00-009 design documents existed without code. M00-009b through e
  now provide deterministic public-state, redacted-view, choice, and resolved-event components.
- Eight focused tests cover canonical state ordering, state byte stability, viewer-private identity
  preservation, opponent redaction, view byte stability, choice option ordering, payload
  canonicalization/refusal, and event UID/cancellation/context projection. Oracle HEAD remains
...
- Next dependency-ready package: M00-009f — executable outcome projection. A later narrow exporter-
...
|---|---|---|
| M00 Oracle and baseline | Written | **Partial** — corpus imported and checksummed; deterministic public-state, redacted-view, choice, and resolved-event projections are executable. No complete oracle exporter, generated fixtures, or differential corpus. Correctness baseline was only collected, never run. Performance baseline disputed (see audit). |
...
1. **The oracle exporter is incomplete.** M00-009b/c/d/e provide tested state, redacted-view,
   choice, and resolved-event projections, but no CLI/NDJSON stream, fixture manifest, outcomes,
   error projections, or complete reproducibility campaign exists. Until those are complete, no
