...
| M09-027b | Critic vocabulary reachability | 027 | MLP plan §§4.1, 4.5 | OOV registry v3 appending `critic-state`; a projection role for the family; the corpus emitting critic names; one regenerated generation. **Recorded before implementation because M09-027 measured the gap:** against the accepted generation every `critic-state:*` name is unassigned and resolves to the global OOV column, so `V` is a rank-1 projection of the position until this lands. |
| M09-028 | Schema-6 inference bundle | 024–027b | MLP plan §§4.4–4.6 | Safe tensor bundle, manifest-last atomic write, bounded validation/recovery, CPU round trip; training resume remains M10-035. |
